...
int read_sector(void *buf, uint32_t sector) {
    if (fseek(in, (long)sector * bs.sector_size, SEEK_SET) != 0) return -1;
    return fread(buf, 1, bs.sector_size, in);
}

int write_sector(void *buf, uint32_t sector) {
    if (fseek(in, (long)sector * bs.sector_size, SEEK_SET) != 0) return -1;
    return fwrite(buf, 1, bs.sector_size, in);
}
...
void set_fat_entry(uint16_t cluster, uint16_t value) {
    ...
    if (bs.number_of_fats > 1) {
        fseek(in, offset + (bs.fat_size_sectors * bs.sector_size), SEEK_SET);
        fwrite(&value, 2, 1, in);
    }
}
...
int fat_read_data(const Fat16Entry *entry, char *buf, size_t size, off_t offset) {
    ...
    size_t total = 0;
    unsigned char *tmp = malloc(cluster_size);
    while (total < size && cluster < 0xFFF8) {
        read_sector(tmp, get_cluster_offset(cluster) / bs.sector_size);
...
int fat_create_file(const char *path) {
    ...
    entry->modify_date = ((tm_info->tm_year - 80) << 9) | ((tm_info->tm_mon + 1) << 5) | tm_info->tm_mday;
    entry->modify_time = (tm_info->tm_hour << 11) | (tm_info->tm_min << 5) | (tm_info->tm_sec / 2);

    write_sector(buf, dir_sector);
    return 0;
}

int fat_write_data(const char *path, const char *buf, size_t size, off_t offset) {
    Fat16Entry entry;
